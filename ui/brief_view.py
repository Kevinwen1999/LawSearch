...
def generated_at(brief: dict) -> str:
    """'2026-09-26 05:40 UTC': with the time, so a same-day regenerate visibly changes it."""
    stamp = brief["created_at"]
    return f"{stamp[:10]} {stamp[11:16]} UTC"


def brief_items(brief: dict) -> list[str]:
    """The texts of the five-part brief (not the full reading), for comparing two versions."""
    summary = brief["summary"]
    items = []
    for issue in summary["issues"]["items"]:
        items += [issue["question"], *(s["question"] for s in issue["sub_issues"])]
    items += [f["text"] for f in summary["facts"]["items"] if f["kind"] == "event"]
    items += [r["text"] for r in summary["ratio"]["items"]]
    items += [d["answer"] for d in summary["decision"]["items"]]
    return items


def regenerated_note(old: dict, new: dict) -> str:
    before, after = brief_items(old), brief_items(new)
    changed = sum(1 for item in after if item not in before)
    if changed == 0 and len(before) == len(after):
        what = "The brief reads the same as before."
    else:
        what = f"{changed} of {len(after)} brief items differ from the previous version."
    return (
        f"New brief written at {generated_at(new)}. {what} Briefs use the court's own words, so a "
        "new version of the same decision often differs only in a few words."
    )
...
def render_brief(brief: dict, *, show_refs: bool = True, show_full: bool = True) -> None:
    ...
    with st.container(horizontal=True, vertical_alignment="center"):
        ...
        st.caption(
            ...
            f"prompt {brief['prompt_version']} · generated {generated_at(brief)}"
        )
