...
| 2026-09-24 | `/scenarios` merges the combined query and each issue's results into k slots (combined top 3, then round-robin) | Same top k, then each issue's own top `issue_k` (3) as labelled groups; the flat list keeps the old top k first | An external multi-issue eval (Ontario wrongful dismissal, now `on-003`/`on-004`) found the per-issue searches ranking *Matthews*, *Wallace*, *Honda* at #1–2 on their issues, but 8 issues shared 7 slots after the combined lead. New `scripts/eval_scenarios.py` (fingerprints cached in `eval/fingerprints/`): top 10 unchanged, recall@25 tune 0.491 → 0.531, test 0.583 → 0.608. Leading with the combined query's top 8 instead lowered test nDCG@10 0.400 → 0.340. The eval's other findings are tracked in IMPROVEMENTS.md |
| 2026-09-25 | Pre-2007 ONCA decisions (docket-number citations) had no in-corpus citations | `load_citations` also resolves name-and-year citations ("Hobbs v. TDI Canada Ltd. (2004), 246 D.L.R. (4th) 43 (Ont. C.A.)") on (last word before "v.", first word after, year), only with an Ontario Court of Appeal marker ((Ont. C.A.), (C.A.), O.A.C., (ON CA)) in the same citation; keys shared by two decisions are dropped | All 6,201 of them had `cited_by_count = 0`, so graph expansion and the citation prior never saw them (*Hobbs* ranked #121 on its issue). Now 1,584 have citations (6,507 edges); a 30-citation hand sample was all correct. Citations naming no court ("(2004), 246 DLR (4th) 43. There, this court…") stay unresolved on purpose |
| 2026-09-25 | Ontario regulations: not covered (A2AJ has Acts only) | `scripts/ingest_ontario_regulations.py` loads the regulations the corpus cites (2+ decisions: 132; 97 current on e-Laws, 12,015 section chunks) from e-Laws' JSON API; matched in text by citation, by title only when 3+ words, and the Rules of Civil Procedure by rule ("r. 20.04"); `/scenarios` flags cited regulations still not loaded | O. Reg. 288/01's "wilful misconduct" standard decides for-cause termination clauses. After re-linking: 337,638 case→section edges (was 322,844); r. 21.01 cited by 251 decisions, r. 20.04 by 58. Mirroring all of e-Laws was rejected: most regulations are never cited |
| 2026-09-25 | Authorities cited by the top cases but not in the corpus vanished silently | `POST /canlii/cited`: CanLII citator's cited-cases lists for the scenario's seeds; authorities cited by 2+ seeds that the results don't show, labelled in LawSearch or not | *Bardal v. Globe & Mail* (named in 66 corpus decisions, no row) is now listed for on-002/003/004. The in-corpus side (e.g. *Machtinger* when not retrieved) comes free. ~8 CanLII queries per uncached scenario |
| 2026-09-25 | Fingerprint prompt v3; gate asked whenever the model listed a clarification | Prompt v4: remedies/defences (mitigation, limitation periods) as their own issues, up to 10 issues; `needs_clarification` only for an unstated jurisdiction, never missing facts; criminal/Charter = federal. The gate blocks only when jurisdiction is `unknown`; otherwise the question comes back as a note | Sonnet (v3) stopped 12 of 36 eval scenarios, mostly asking for dates or side issues; Qwen (v4) stops 5, each a private-law question with no province stated |
| 2026-09-25 | LM Studio backend ignored the effort setting | Passes `reasoning_effort` (low/medium/high; xhigh/max → high) | Without it Qwen reasoned until the 8,000-token budget ran out (38k chars, no answer) and every fingerprint fell back to Sonnet; at "low" it answers in ~35 s |
| 2026-09-25 | OCR: RapidOCR's bundled (Chinese) recognizer, text-direction classifier on | PaddleOCR's English recognizer (`en_PP-OCRv3_rec`, fetched once from RapidOCR's HF repo), classifier off | The bundled model dropped every space between English words and the classifier flipped some upright lines into gibberish: scanned-PDF word agreement 0.25 → 0.998 across all 37 eval scenarios (`scripts/eval_intake.py`) |
| 2026-09-25 | CanLII daily cap counted per UTC day | Hourly buckets, cap enforced over any rolling 24 hours (`canlii_usage_hourly`) | CanLII doesn't say when its day starts; staying under the cap in every 24-hour window keeps it under on every calendar day |
| 2026-09-25 | Scenario fingerprints recomputed every request | `fingerprint_cache` table keyed by text hash, prompt version and configured model | Re-running or re-uploading a scenario skips the LLM call; a prompt or model change bypasses old entries |
| 2026-09-25 | (tried) "Often cited together" sibling sections from case co-citation | Not built | ESA s. 65 is cited by 5 decisions and s. 61 by 2, too sparse to find s. 64/60 as siblings, and the query took 12 s where the Charter was involved. Per-issue legislation groups and the regulations cover the "related provisions" gap instead |
| 2026-09-25 | (tried) Rerank short decisions on every chunk | Not adopted; reranking still sees the retrieved passages only | *Waksdale* scored 0.9 on an issue query because its retrieved passages were its facts, while its ¶3 scores 2.65. Reranking every chunk of decisions with <= 15 chunks fixed that case (#15 -> #9) but lowered fingerprinted test nDCG@10 0.370 -> 0.343 (`eval/runs/scenarios-backlog-short15.json`); adding the chunk closest to the query embedding changed nothing (it wasn't ¶3 either). With the v4 fingerprint, Waksdale ranks #3 on its issue anyway |
| 2026-09-25 | Per-issue case gate 0.0; all statute sections eligible | Case gate -2.0; sections no decision cites are dropped unless the scenario names their law | Fingerprinted eval, 36 scenarios: gate -2 best on tune (nDCG@10 0.410 -> 0.420, issue coverage kept) and better on test (0.370 -> 0.383, recall@25 0.615 -> 0.656). Dropping uncited sections left legislation recall unchanged and roughly halved the uncited sections shown on test |
| 2026-09-25 | `/scenarios` searched only the fingerprint's queries (combined + per issue) | The scenario's own text (first 4,000 chars) is searched too and leads the best-overall list; the fingerprint's combined query becomes one more group ("More matches for the scenario as a whole") | The P6 with/without-fingerprint comparison (now measurable: `scripts/eval_scenarios.py` vs `eval_retrieval.py` on the same 36 scenarios) showed raw-text search beating the fingerprinted pipeline on test nDCG@10 (0.477 vs 0.383). Leading with it: test 0.383 -> 0.439, legislation recall tune 0.624 -> 0.675 and test 0.742 -> 0.803, tune nDCG@10 a near-tie (0.420 -> 0.417). One extra search per scenario (~1-2 s) |
| 2026-09-25 | Scenario text leads the best-overall list with its top 3 (COMBINED_LEAD) | Its top 7 (`SCENARIO_TEXT_LEAD`); the fingerprint-only path keeps 3 | Re-scored on the same searches: nDCG@10 tune 0.417 -> 0.427, test 0.439 -> 0.483 (raw-text search alone: 0.406 / 0.477); lead 5 and 10 were below 7 on both splits; legislation and issue coverage unchanged |
| 2026-09-25 | Statute matcher: "para."/"clause" continued a section list | Only section-level words continue a list; a paragraph/clause right after a reference belongs to it | "O. Reg. 288/01, s. 2(1), para. 3" linked s. 3 as well as s. 2 (same for Acts); standalone "paragraph 18(1)(a) of the ITA" still resolves to s. 18 |
